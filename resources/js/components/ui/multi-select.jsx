...
export const MultiSelect = React.forwardRef(
	(
		...
		const filteredOptions = React.useMemo(() => {
			...
			return options.filter(
				(option) =>
					...
					typeof option.value === 'string' && option.value.toLowerCase().includes(searchValue.toLowerCase())
			);
		...
		const toggleOption = (optionValue) => {
			...
			const isCurrentlySelected = isValueSelected(optionValue);
			...
			if (setValues) {
				const selectedOptions = newSelectedValues.map(value =>
					getAllOptions().find(opt => opt.value === (typeof value === 'object' ? value.value : value))
				).filter(Boolean);
				...
				if (searchValue && isPopoverOpen) {
					const filteredCount = allOptions.filter(
						(opt) =>
							...
							typeof opt.value === 'string' &&  opt.value.toLowerCase().includes(searchValue.toLowerCase())
					).length;
		...
		}, [selectedValues, isPopoverOpen, searchValue, announce, getAllOptions, getSelectedValueKeys]);
